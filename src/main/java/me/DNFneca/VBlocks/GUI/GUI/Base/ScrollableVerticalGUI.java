...
public class ScrollableVerticalGUI extends GUI {
    ...
    public ScrollableVerticalGUI(Component title, int size, Map<Integer, GUIItem> pattern, List<RegistryReference<GUIPlaceOption>> placementOptions) {
        super(title, size);
    ...
    public ScrollableVerticalGUI(Component title, int size, Map<Integer, GUIItem> pattern) {
        ...
        getPlacementOptions().addField(SCROLLABLE_VERTICAL_NEXT);
        getPlacementOptions().addField(SCROLLABLE_VERTICAL_BACK);
        getPlacementOptions().addField(SHOULD_PLACE_SEARCH);
    ...
    public void open(Player player) {
        ...
        for (int column = 0; column < columns; column++) {
            for (int row = 0; row < 7; row++) {
                int index = offset + ((column + 1) * 7 - row);
                int slot = ((column + 1) * 7 - row) + column * 2;
